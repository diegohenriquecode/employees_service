...
import {EvaluationRemoveOnSectorEventMessage} from '../schema';
...
async function recordHandler(record: SQSEvent['Records'][0]) {
    ...
    const {account: account_id, sector, type, user} = JSON.parse(Message) as EvaluationRemoveOnSectorEventMessage;

...
async function _handler(event: SQSEvent) {
    for (const record of event.Records) {
        ...
        if (payload.EventType === 'INSERT') {
            ...
            await repository.create(dismissInterview);
        } else if (payload.EventType === 'MODIFY') {
            const {OldItem: dismissInterview, NewItem: patch} = payload;
            await repository.update(dismissInterview, patch);
        } else if (payload.EventType === 'REMOVE') {
            const {OldItem: dismissInterview} = payload;
            await repository.remove(dismissInterview.id);
        }
        await queues.deleteMessage(record);
    }
}

const repository = DismissInterviewsMysqlRepository.config(config);
